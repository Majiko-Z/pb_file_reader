...
mod test {
    ...
    #[test]
    fn test_multiple_dbf_manager() {
        //  开多个线程接收同一CSV的数据
        let path_1 = PathBuf::from("/Users/yaohui/projects/pb_file_reader/test/data.dbf");
        let path_1_c1 = path_1.clone();
        let path_1_c2 = path_1.clone();

        let h1 = std::thread::spawn(move || {
            let mut cnt = 1;
            let reader = get_or_create_dbf_reader::<TestDbfStruct1>(&path_1, true, EncType::UTF8);
            if reader.is_err() {
                return;
            }
            let reader = reader.unwrap(); 
            let (cert_key, recv_chan) = reader.subscribe("0", |verify, data| -> bool {
                (data.a  as i32 % 2).to_string() == verify
            });
            ...
            remove_dbf_reader::<TestDbfStruct1>(cert_key, true, &path_1);
        });
